...
void lcd_printPGM_utf(const char *str, uint8_t n=LCD_WIDTH) {
  ...
  while (n && (c = pgm_read_byte(str))) n -= charset_mapper(c), ++str;
    ...
    void lcd_custom_bootscreen() {
      constexpr u8g_uint_t left = (LCD_PIXEL_WIDTH  - (CUSTOM_BOOTSCREEN1_BMPWIDTH)) / 2,
                           top = (LCD_PIXEL_HEIGHT - (CUSTOM_BOOTSCREEN1_BMPHEIGHT)) / 2;
      #if ENABLED(CUSTOM_BOOTSCREEN1_INVERTED)
        constexpr u8g_uint_t right = left + CUSTOM_BOOTSCREEN1_BMPWIDTH,
                             bottom = top + CUSTOM_BOOTSCREEN1_BMPHEIGHT;
      #endif
      u8g.firstPage();
      do {
        u8g.drawBitmapP(left, top, CEILING(CUSTOM_BOOTSCREEN1_BMPWIDTH, 8), CUSTOM_BOOTSCREEN1_BMPHEIGHT, custom_start_bmp1);
        #if ENABLED(CUSTOM_BOOTSCREEN1_INVERTED)
          u8g.setColorIndex(1);
          if (top) u8g.drawBox(0, 0, LCD_PIXEL_WIDTH, top);
          if (left) u8g.drawBox(0, top, left, CUSTOM_BOOTSCREEN1_BMPHEIGHT);
          if (right < LCD_PIXEL_WIDTH) u8g.drawBox(right, top, LCD_PIXEL_WIDTH - right, CUSTOM_BOOTSCREEN1_BMPHEIGHT);
          if (bottom < LCD_PIXEL_HEIGHT) u8g.drawBox(0, bottom, LCD_PIXEL_WIDTH, LCD_PIXEL_HEIGHT - bottom);
        #endif
      } while (u8g.nextPage());
      safe_delay(CUSTOM_BOOTSCREEN1_TIMEOUT);
      u8g.firstPage();
      do {
        u8g.drawBitmapP(left, top, CEILING(CUSTOM_BOOTSCREEN2_BMPWIDTH, 8), CUSTOM_BOOTSCREEN2_BMPHEIGHT, custom_start_bmp2);
        #if ENABLED(CUSTOM_BOOTSCREEN2_INVERTED)
          u8g.setColorIndex(1);
            if (top) u8g.drawBox(0, 0, LCD_PIXEL_WIDTH, top);
            if (left) u8g.drawBox(0, top, left, CUSTOM_BOOTSCREEN2_BMPHEIGHT);
            if (right < LCD_PIXEL_WIDTH) u8g.drawBox(right, top, LCD_PIXEL_WIDTH - right, CUSTOM_BOOTSCREEN2_BMPHEIGHT);
            if (bottom < LCD_PIXEL_HEIGHT) u8g.drawBox(0, bottom, LCD_PIXEL_WIDTH, LCD_PIXEL_HEIGHT - bottom);
        #endif
      } while (u8g.nextPage());
      safe_delay(CUSTOM_BOOTSCREEN2_TIMEOUT);
    }
